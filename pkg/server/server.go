...
type Server struct {
	...
	// and snapshot TTL expiry and changelog polling are disabled.
...
// skips the bounded-staleness safeguards (snapshot TTL expiry and changelog
// polling) that are otherwise enabled by default for multi-instance
// deployments.
...
func NewServerWithOpts(opts ...OpenFGAServiceV1Option) (*Server, error) {
	...
	reachabilityOptions := []reachability.Option{reachability.WithLogger(s.logger)}
	if s.reachabilityIndexSingleWriter {
		...
		// Writes may happen on other processes sharing the datastore. Bound
		// staleness with a snapshot TTL and use changelog polling as a
		// best-effort fast invalidation path.
		reachabilityOptions = append(reachabilityOptions,
			reachability.WithSnapshotTTL(s.reachabilityIndexSnapshotTTL),
			reachability.WithChangelog(s.datastore),
		)
	}
